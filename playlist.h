 ...
 * @pre O arquivo binário de playlists deve estar aberto em modo "rb+". O código de playlist fornecido deve ser único.
 * @pos A nova playlist é persistida no arquivo e o cabeçalho é atualizado.
 */
int criar_playlist(FILE *arq_play, int codigo, const char *titulo);

/**
 * @brief Imprime o código e o título de todas as playlists cadastradas[cite: 31].
 * Propósito: Percorre de forma encadeada o arquivo de playlists exibindo seus dados básicos identificadores[cite: 31].
 * @pre O arquivo de playlists deve estar aberto em modo "rb".
 * @pos A lista resumida (código e título) de todas as playlists criadas é impressa na tela[cite: 31].
 */
void imprimir_lista_playlists(FILE *arq_play);

/**
 * @brief Imprime as músicas de uma playlist detalhadamente[cite: 30].
 ...
 * @pos Imprime o título da playlist seguido pelos títulos e artistas das músicas associadas, na ordem correta[cite: 30].
 */
void imprimir_playlist_especifica(FILE *arq_play, FILE *arq_faixas, FILE *arq_musicas, int codigo_playlist);

/**
 * @brief Busca interna para localizar uma playlist pelo seu código e retornar sua posição em disco.
 ...
 * @pos Retorna 1 se a playlist for encontrada, preenchendo a estrutura 'saida' e guardando em 'pos_offset' 
 ...
 * @pre O arquivo binário de playlists deve estar aberto em modo "rb+". 'pos_offset' deve ser um deslocamento válido em bytes.
 * @pos O registro da playlist na posição indicada é atualizado com as novas informações em disco.
 */
